...
export class CardRestoredEvent extends TrelloEvent {
    constructor(card: TrelloCard, list: TrelloList, member: TrelloMember, board: TrelloBoard) {
        super(board, TrelloEvents.CARD_RESTORED);
        this.templateString = `{member} restored the card <a href="{card_url}">{card_name}</a> under {list_name}`;
