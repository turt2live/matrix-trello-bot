...
export class CardUpdatedEvent extends TrelloEvent {
    constructor(card: TrelloCard, list: TrelloList, member: TrelloMember, board: TrelloBoard) {
        super(board, TrelloEvents.CARD_UPDATED);
        this.templateString = `{member} updated the card <a href="{card_url}">{card_name}</a> under {list_name}`;
